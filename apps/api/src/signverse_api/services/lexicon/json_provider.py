...
LOCAL_LEXICON_PATH = Path(__file__).resolve().parents[4] / "resources" / "lexicon" / "lexicon.json"
PACKAGED_LEXICON_PATH = (
    Path(__file__).resolve().parents[2] / "resources" / "lexicon" / "lexicon.json"
)
DEFAULT_LEXICON_PATH = LOCAL_LEXICON_PATH if LOCAL_LEXICON_PATH.is_file() else PACKAGED_LEXICON_PATH
...
class JSONLexiconProvider:
    ...
    async def lookupConcept(self, concept: str) -> ISLToken | None:
        entry = self._entries_by_concept.get(concept.strip().casefold())
        return self._to_token(entry) if entry is not None else None
