...
from signverse_api.api.routes.streaming import _send_json
...
@pytest.mark.anyio
async def test_drops_response_when_client_disconnects_during_interpretation() -> None:
    class ClosedWebSocket:
        async def send_json(self, _: object) -> None:
            raise RuntimeError("Unexpected ASGI message after sending websocket.close")

    websocket = ClosedWebSocket()
    assert await _send_json(websocket, {"type": "interpretation"}) is False  # type: ignore[arg-type]
