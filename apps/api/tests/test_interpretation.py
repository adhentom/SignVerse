...
async def test_interpret_accepts_extension_content_packet(client: AsyncClient) -> None:
    ...
    assert response.json() == {
        ...
        "isl_gloss": [],
        "confidence": 0.0,
        "playback": {"items": [], "unsupported_tokens": [], "missing": []},
    }
...
@pytest.mark.anyio
async def test_interpret_rejects_unbounded_metadata(client: AsyncClient) -> None:
    too_many_values = {f"key-{index}": index for index in range(300)}
    deeply_nested: object = "value"
    for _ in range(8):
        deeply_nested = {"child": deeply_nested}

    nodes_response = await client.post("/interpret", json={**PACKET, "metadata": too_many_values})
    depth_response = await client.post(
        "/interpret", json={**PACKET, "metadata": {"root": deeply_nested}}
    )

    assert nodes_response.status_code == 422
