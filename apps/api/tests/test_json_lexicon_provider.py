...
def provider() -> JSONLexiconProvider:
    return JSONLexiconProvider(LEXICON_PATH)
...
@pytest.mark.anyio
async def test_default_provider_uses_public_seed_vocabulary_only() -> None:
    provider = JSONLexiconProvider()

    token = await provider.lookupConcept("beautiful")
    health = await provider.health()
