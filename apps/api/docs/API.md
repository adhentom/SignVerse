...
Accepts one short audio segment as the raw request body. Supported media types are WebM, Ogg,
MP4, MPEG, and WAV. The Chrome extension sends `X-SignVerse-Language: en` and receives:

```json
{
  "text": "The spoken English caption.",
  "language": "en"
}
```

The endpoint is available only when `OPENAI_API_KEY` is configured. Requests are limited to 8 MB,
validated by media type, and processed with the configured OpenAI transcription model. Audio is
not persisted by SignVerse. Empty, oversized, unsupported, timed-out, rate-limited, and provider
failure cases return explicit HTTP errors and never fabricate a caption.
