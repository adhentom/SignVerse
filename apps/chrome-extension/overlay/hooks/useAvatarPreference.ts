  ...
  useEffect(() => {
    ...
    void chrome.storage.local.get([STORAGE_KEY, SCALE_STORAGE_KEY])
      .then((value) => {
        ...
        if (active && !userSelected.current) {
          const migrated = migrateAvatarProfileId(stored);
          setId(migrated);
          if (stored !== undefined && stored !== migrated) {
            void chrome.storage.local.set({ [STORAGE_KEY]: migrated }).catch(() => undefined);
          }
        }
